...
class BaseTrainer:
    def __init__(self, model, loss, resume, config, train_loader, val_loader=None, test_loader=None, train_logger=None):
        ...
        self.csv_log = os.path.join(config['exp_dir'], "result_metrics.csv")
        self.plot_log = os.path.join(config['exp_dir'], "plot.csv")
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs+1):
            ...
            self.moving_miou.append(train_results['Mean_IoU'])
            self.moving_miou.pop(0)
            moving_average = sum(self.moving_miou)/len(self.moving_miou)
            moving_average = round(moving_average, 3)
            ...
            if self.do_validation and epoch % self.config['trainer']['val_per_epochs'] == 0:
                ...
                #if val_results["Mean_IoU"] > self.best_val:
                #    self.best_val = val_results["Mean_IoU"]
                
            print("moving iou", self.moving_miou)
            ...
            if self.mnt_mode != 'off':
                try:
                    #if self.mnt_mode == 'min': self.improved = ( moving_average < self.mnt_best)
                    #else: 
                    self.improved = (moving_average > self.mnt_best)
                except KeyError:
                    self.logger.warning(f'The metrics being tracked ({self.mnt_metric}) has not been calculated. Training stops.')
                    break
                    
                if self.improved:
                    self.mnt_best = moving_average
                    self.not_improved_count = 0
                    with open(self.movingavg_log, "a") as f:
                        writer = csv.writer(f)
                        #writer.writerow(self.moving_miou)
                        writer.writerow([self.config['episode'], epoch, moving_average])
                else:
                    self.not_improved_count += 1
                    
                print("improved", self.improved)
                print("not improved count", self.not_improved_count)
                    
                if self.not_improved_count >= self.early_stoping:
                    with open(self.movingavg_log, "a") as f:
                        writer = csv.writer(f)
                        writer.writerow([self.config['episode'], epoch, moving_average, "Still same...stopping!"])                     
            ...
            if epoch % self.save_period == 0:
                ...
                if self.improved:
                    ...
                    with open(self.plot_log, "a") as f:
                        writer = csv.writer(f)
                        writer.writerow([self.config['episode'], epoch, val_results['Mean_IoU']])
        ...
        for epoch in range(self.start_epoch, self.epochs+1):
            ...
            #wandb.log({'train_miou': results['Mean_IoU'], 'epoch':epoch})
            #wandb.log({'train_class_iou': results['Class_IoU'][1], 'epoch':epoch})
            with open(self.train_iou, "a") as f:
                writer = csv.writer(f)
                writer.writerow([self.config['episode'], epoch, results['Mean_IoU']])
            if self.do_validation and epoch % self.config['trainer']['val_per_epochs'] == 0:
                results = self._valid_epoch(epoch)
                #wandb.log({'val_miou': results['Mean_IoU'], 'epoch':epoch})
                #wandb.log({'val_class_iou': results['Class_IoU'][1], 'epoch':epoch})
            ...
            if epoch % self.save_period == 0:
                ...
                if self.improved:
                    ...
                    #wandb.log({'test_miou': test_results['Mean_IoU'], 'epoch':epoch})
                    #wandb.log({'test_class_iou': test_results['Class_IoU'][1], 'epoch':epoch})
