...
from sampling.coreset import Coreset_Sampling
...
def create_expdir(cfg, args):
    ...
    print(cfg['val_loader']['args']['load_from'])
    return cfg
...
def al():
    ...
    elif args.sampling_strategy == 'Coreset':
        strategy = Coreset_Sampling()
    ...
    while episode < args.max_episode:
        '''
        run = wandb.init(project="Benchmark", reinit=True)
        with open(os.path.join(config['exp_dir'], "run_name.csv"), "a") as f:
            writer = csv.writer(f)
            writer.writerow([wandb.run.name])
        '''
...
if __name__=='__main__':
    al()
    #wandb.alert(title="Training completed", text="Training for random completed")
