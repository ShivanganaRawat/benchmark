...
class ResNet(nn.Module):
    def __init__(self, drop_conv=0.0, in_channels=3, output_stride=16, backbone='resnet101', pretrained=True):
        ...
        if not pretrained or in_channels != 3:
            ...
            self.layer0 = nn.Sequential(*list(model.children())[:5])
...
def assp_branch(in_channels, out_channles, kernel_size, dilation, drop_conv):
...
class ASSP(nn.Module):
    def __init__(self, in_channels, output_stride, drop_conv=0.0):
        ...
        self.aspp1 = assp_branch(in_channels, 256, 1, dilation=dilations[0], drop_conv=drop_conv)
        self.aspp2 = assp_branch(in_channels, 256, 3, dilation=dilations[1], drop_conv=drop_conv)
        self.aspp3 = assp_branch(in_channels, 256, 3, dilation=dilations[2], drop_conv=drop_conv)
        self.aspp4 = assp_branch(in_channels, 256, 3, dilation=dilations[3], drop_conv=drop_conv)
...
class Decoder(nn.Module):
    def __init__(self, low_level_channels, num_classes, drop_conv=0.0):
...
class DeepLab(BaseModel):
    def __init__(self, num_classes, drop_conv=0.0, in_channels=3, backbone='xception', pretrained=True, 
                output_stride=16, freeze_bn=False, freeze_backbone=False,  **_):
                
        super(DeepLab, self).__init__()
        assert ('xception' or 'resnet' in backbone)
        if 'resnet' in backbone:
            self.backbone = ResNet(in_channels=in_channels, output_stride=output_stride, pretrained=pretrained, drop_conv=drop_conv)
            low_level_channels = 256
        else:
            self.backbone = Xception(output_stride=output_stride, pretrained=pretrained)
            low_level_channels = 128

        self.ASSP = ASSP(in_channels=2048, output_stride=output_stride, drop_conv=drop_conv)
        self.decoder = Decoder(low_level_channels, num_classes, drop_conv=drop_conv)
