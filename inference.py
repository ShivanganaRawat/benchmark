...
def main():
    ...
    #assert dataset_type in ['VOC', 'COCO', 'CityScapes', 'ADE20K', "Leaf", "Tomato"]
    ...
    normalize = transforms.Normalize([0.24152601, 0.23049359, 0.17779869], [0.19978444, 0.18484720, 0.15231922])
    num_classes = 2
    ...
    val_loader = get_instance(dataloaders, 'test_loader', config)
    ...
    with torch.no_grad():
        for img_idx, (data, target) in enumerate(tbar):
            ...
            output = F.softmax(torch.from_numpy(output)).argmax(0).cpu()
            output = output.numpy()
            print(output.shape, np.unique(output))
            print(val_loader.dataset[img_idx])
            save_images(data, output, args.output, str(img_idx)+".png", palette)
    '''

    if not os.path.exists('outputs'):
        os.makedirs('outputs')

    image_files = sorted([f for f in os.listdir(args.images) if not f.startswith('.')])
    with torch.no_grad():
        tbar = tqdm(image_files, ncols=100)
        for img_file in tbar:
            image = np.asarray(Image.open(os.path.join(args.images, img_file)), dtype=np.float32)[:, :, :3]
            ...
            print(torch.from_numpy(prediction).shape)
...
def parse_arguments():
    ...
    parser.add_argument('-mo', '--mode', default='simple', type=str,
